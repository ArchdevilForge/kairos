"""Main CLI entry point for kairos."""
...
def validate_exchange(value: str, language: str) -> tuple[bool, str]:
    ...
    from kairos.utils.default_symbols import VALID_EXCHANGES, get_prompt
...
def validate_timeframe(value: str, language: str) -> tuple[bool, str]:
    ...
    from kairos.utils.default_symbols import VALID_TIMEFRAMES, get_prompt
...
def validate_positive_number(value: str, language: str) -> tuple[bool, float | str]:
    ...
    from kairos.utils.default_symbols import get_prompt
...
def update_markets(config):
    ...
    from kairos.utils.supported_markets import refresh_supported_markets
...
async def run_monitoring():
    ...
    from kairos.notifications.telegram_bot_service import TelegramBotService
...
def cmd_update_markets(args):
    ...
    from kairos.utils.setup_logging import setup_logging
    from kairos.utils.supported_markets import list_cached_exchanges, refresh_supported_markets
...
def _run_start_preflight() -> None:
    ...
    if "telegram" in config.get("notificationChannels", []):
        if not _validate_telegram_token(config):
            ...
            print("请检查 ~/.config/kairos/config.yaml 中的 telegram.token 设置")
        ...
        if not telegram_chat_id:
            ...
            print("请检查 ~/.config/kairos/config.yaml 中的 telegram.chatId 设置")
            sys.exit(1)
...
def cmd_start(_args):
    ...
    if running_pid is not None:
        print(f"kairos is already running in background (PID: {running_pid})")
    ...
    if process.poll() is not None:
        raise SystemExit("Failed to start kairos in background")

    _write_pid_file(process.pid)
    print(f"kairos started in background (PID: {process.pid})")
    print(f"Log: {log_path}")
...
def cmd_status(_args):
    ...
    if running_pid is None:
        print("kairos is not running")
        return
    print(f"kairos is running (PID: {running_pid})")
...
def cmd_stop(_args):
    ...
    try:
        ...
        pass
    print(f"kairos stopped (PID: {running_pid})")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(prog="kairos", description="Cryptocurrency futures price monitor & trading system")
    ...
    # Merge commands
    all_commands = {**monitoring_commands, **trading_commands}

    handler = all_commands.get(args.command, cmd_start)
    handler(args)
