...
func (s *MarketScanner) fetchAllTickers(ctx context.Context, exch exchange.Exchange) map[string]*types.Ticker {
	tickers, err := exch.FetchTickers(ctx)
	if err != nil {
		log.Printf("scanner: failed to fetch tickers: %v", err)
		return nil
	}
	return tickers
}
