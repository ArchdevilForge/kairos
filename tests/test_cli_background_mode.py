...
def test_get_pid_and_log_paths_live_in_config_dir(tmp_path, monkeypatch):
    monkeypatch.setattr("kairos.paths.get_config_dir", lambda: tmp_path)

    from kairos.paths import get_log_path, get_pid_path

    assert get_pid_path() == tmp_path / "kairos.pid"
    assert get_log_path() == tmp_path / "kairos.log"


def test_cmd_start_launches_background_process_and_writes_pid(monkeypatch, tmp_path, capsys):
    pid_path = tmp_path / "kairos.pid"
    log_path = tmp_path / "kairos.log"
    ...
    monkeypatch.setattr(cli, "_get_runner_module", lambda: "kairos.app.runner")
    monkeypatch.setattr("kairos.app.cli.get_pid_path", lambda: pid_path)
    monkeypatch.setattr("kairos.app.cli.get_log_path", lambda: log_path)
...
def test_cmd_status_reports_running_process(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kairos.app.cli.get_log_path", lambda: Path("/tmp/kairos.log"))
...
def test_runner_uses_file_logging_without_console_duplication():
    import inspect
    import kairos.app.runner as runner
